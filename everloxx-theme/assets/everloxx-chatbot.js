...
const STORAGE_KEY = "everloxx_chat_history_v1";
...
function getTurnstileSiteKey() {
  return (window.__EVERLOXX_CONFIG && window.__EVERLOXX_CONFIG.turnstileSiteKey) || (window.__EVERLOXX_CONFIG && window.__EVERLOXX_CONFIG.turnstileSiteKey) || "";
}
...
async function addToCart(handle, quantity = 1) {
  try {
    ...
    console.error('[EVERLOXX] addToCart error:', e);
...
function renderButtons(buttonsJson, container) {
  try {
    ...
    btnWrap.className = "everloxx-buttons";

    data.buttons.forEach(btn => {
      const el = document.createElement("button");
      el.className = "everloxx-btn";
      if (btn.variant === "preferred") el.classList.add("everloxx-btn-preferred");
      if (btn.variant === "primary") el.classList.add("everloxx-btn-primary");
      ...
      el.addEventListener("click", () => {
        ...
        document.querySelectorAll(".everloxx-buttons").forEach(g => {
...
/* ===== Auto-Greeting ===== */
const GREETING_TEXT = `Hallo 👋 Willkommen bei EVERLOXX Design!
